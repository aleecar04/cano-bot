...
def _parse(msg):
    """Convierte la respuesta del testbot a dict. Los plugins control/scanner
    devuelven dict que errbot serializa como repr() — no JSON válido."""
...
class TestListDevices:
    ...
    def test_includes_device_names(self, testbot):
        ...
        names = [d["name"] for d in _parse(msg)["dispositivos"]]
...
class TestControlDevice:
    ...
    def test_invalid_json_args_returns_error(self, testbot):
        ...
        result = _parse(testbot.pop_message())
        assert result["ok"] is False
