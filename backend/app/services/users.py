...
class UsersService:
    ...
    def resolve_jid_in_house(self, jid: str, house_id: str) -> str | None:
    ...
    def resolve_username_to_email(self, username: str) -> str:
    ...
    async def create_user_with_xmpp(
        ...
        try:
            xmpp_password = await xmpp_service.create_xmpp_account(username=username)
    ...
    async def register(self, user_in: UserRegister) -> dict:
        ...
        return await self.create_user_with_xmpp(
            email=user_in.email,
    ...
    async def regenerate_xmpp_password(self, user_id: str) -> dict:
        jid = xmpp_account_repository.find_jid_by_user(user_id)
        if not jid:
            raise not_found("Cuenta XMPP no encontrada")
        new_password = secrets.token_urlsafe(16)
        await xmpp_service.change_xmpp_password(jid, new_password)
