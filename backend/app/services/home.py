...
class HomeService:
    ...
    def generate_invitation_code(self, house_id: str, created_by: str) -> str:
    ...
    def consume_invitation_code(self, code: str, user_id: str) -> dict:
    ...
    def get_user_house(self, user_id: str) -> dict | None:
        house_id = house_member_repository.find_house_id_by_user(user_id)
        if not house_id:
            return None
        return house_repository.find_by_id(house_id)

    def get_house_id_for_user(self, user_id: str) -> str | None:
        return house_member_repository.find_house_id_by_user(user_id)

    def get_bot_target_for_user(self, user_id: str) -> str | None:
        from app.core.config import settings
        house_id = self.get_house_id_for_user(user_id)
        if not house_id:
            return None
    ...
    def require_house(self, user_id: str) -> str:
        house_id = self.get_house_id_for_user(user_id)
        if not house_id:
    ...
    def get_house_member_ids(self, user_id: str) -> list[str]:
        house_id = self.get_house_id_for_user(user_id)
        if not house_id:
            return [user_id]
    ...
    def get_house_with_detail(self, user_id: str) -> dict | None:
        house = self.get_user_house(user_id)
        if not house:
            return None
    ...
    def create_floor(self, house_id: str, name: str) -> dict:
        result = (
    ...
    def get_rooms_by_house(self, house_id: str) -> list[dict]:
    ...
    def create_room(self, floor_id: str, name: str) -> dict:
        result = (
    ...
    def delete_room(self, user_id: str, floor_id: str, room_id: str) -> None:
        if self.get_user_role(user_id) != "owner":
            raise forbidden(_NOT_OWNER_MSG)
        supabase.table("rooms").delete().eq("id", room_id).eq("floor_id", floor_id).execute()

    def delete_floor(self, user_id: str, floor_id: str) -> None:
        house = self.get_user_house(user_id)
        if not house:
    ...
    def list_house_members(self, user_id: str) -> list[dict]:
    ...
    def kick_member(self, owner_id: str, target_user_id: str) -> None:
        if self.get_user_role(owner_id) != "owner":
            raise forbidden(_NOT_OWNER_MSG)
        if target_user_id == owner_id:
            raise bad_request("No puedes expulsarte a ti mismo. Usa la opción de salir.")
        house_id = self.get_house_id_for_user(owner_id)
        (
    ...
    def leave_house(self, user_id: str) -> None:
        house_id = self.get_house_id_for_user(user_id)
        if not house_id:
            return

        was_owner = self.get_user_role(user_id) == "owner"
    ...
    def setup_house(self, user_id: str, name: str = _DEFAULT_HOUSE_NAME) -> dict:
        if self.get_house_id_for_user(user_id):
    ...
    def regenerate_bot_token(self, user_id: str) -> str:
        self.require_owner(user_id)
        house_id = self.require_house(user_id)
