...
class SamsungTVDriver(BaseDriver):
    ...
    def get_status(self, device: dict, timeout: float = 2.0) -> dict:
        try:
            ...
            power = info.get("device", {}).get("PowerState", "off") == "on"
            return {
                "is_online": True,
                "estado": {"power": power, "ultimo_update": time.time()}
            }
        except Exception:
            return {"is_online": False}
    ...
    def ejecutar(self, device: dict, accion: str, payload: dict = {}) -> dict:
        extras = {
            "subir_volumen": lambda: self._send_key(device, "KEY_VOLUMEUP"),
            "bajar_volumen": lambda: self._send_key(device, "KEY_VOLUMEDOWN"),
            "mute":          lambda: self._send_key(device, "KEY_MUTE"),
